...
/** The Node.js builtins `@nonnajs/di`'s published dist bundle statically imports. */
...
export interface NonnaPluginOptions {
     ...
     * and know it never reaches the specific `@nonnajs/di` feature backed by a given shim (see
...
/**
 * Vite plugin that aliases the handful of Node.js builtins `@nonnajs/di`'s published dist bundle
 ...
 * modules, so a browser bundler (Rollup, via Vite) has something to resolve `@nonnajs/di` against
 * at all.
 *
 * @remarks
 * None of these shims provide real Node semantics - they exist purely so the bundler has
 * *something* to resolve, and `@nonnajs/di`'s browser-relevant surface (`Injector.get()`/
